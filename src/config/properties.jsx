export const properties = [
  {
    title: 'Management & maintenance',
    description:
      'RM Plaza is a business hub in Anambra state under one of our property management portfolios',
    image: (
      <img
        className='h-[288px] w-full'
        src='/assets/anambra.jpeg'
        alt='house in anambra'
      />
    ),
    location: 'Anambra, Nigeria',
    purpose: 'For maintenance',
    id: 1,
  },
  {
    title: 'Build & Sale Acquisition Abuja',
    description:
      'Clusters of our property development investment under build and sale.',
    image: <img src='/assets/uncompleted.jpeg' alt='house in abuja' />,
    location: 'Abuja, Nigeria',
    purpose: 'For sale',
    id: 2,
  },
  {
    title: 'Property in Abuja',
    description: 'Located in Abuja',
    image: <img src='/assets/maralago.jpeg' alt='house in abuja' />,
    location: 'Abuja, Nigeria',
    purpose: 'For maintenance',
    id: 3,
  },
];
